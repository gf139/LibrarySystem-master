...
<%--                        <li><a><i class="icon-font">&#xe005;</i>图书管理</a></li>--%>
                        ...
                        <li><a href="/book.do?type=viewOverdue">查看逾期记录</a></li> <!-- 新增的链接 -->
                        <li><a href="/userManagement.do?type=query">用户管理</a></li> <!-- 新增的链接 -->
                    </ul>
                </li>
            </ul>
        </div>
    ...
    <div class="main-wrap">
            ...
            <div class="search-content">
                <form method="get" action="/book.do" id="searchForm">
                    ...
                    <table class="search-tab">
                        <tr>
                            <th width="70">书名:</th>
                            <td><input class="common-text" placeholder="" name="bookname" id="bookname"
                                       value="${type=="search"?formerKeywords:""}" type="text" style="width:150px"></td>
                            <th width="80">选择分类:</th>
                            <td>
                            ...
                            <th width="80">价格区间:</th>
                            <td>
                                <input type="number" name="minprice" id="minprice" min="0" max="1000" placeholder="Min"
                                       value="${type=="search"?formerMinprice:""}" class="common-text">--
                                <input type="number" name="maxprice" id="maxprice" min="0" max="1000" placeholder="Max"
                                       value="${type=="search"?formerMinprice:""}" class="common-text">
                            </td>
                            <th width="80">出版日期:</th>
                            <td>
                                <input type="date" name="minPdate" min="1900-01-01" id="minPdate"
                                       value="${type=="search"?formerMinpdate:""}" class="common-text">--
                                <input type="date" name="maxPdate" min="1900-01-01" id="maxPdate"
                                       value="${type=="search"?formerMaxpdate:""}" class="common-text">
                            </td>
                            <td style="padding-left:50px">
                                <button class="btn btn-primary btn2" type="button" onclick="searchSubmit()">查询
                                </button>
                            </td>
                        </tr>
                    </table>
            ...
            <form name="myform" id="myform" method="post">
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <tr>
                            <td class="tc"><input name="ids[]" value="${i.id}" type="checkbox" class="singleChoose">
                            </td>
                            ...
                            <td align="center">
                                <a class="link-update btn btn-success btn2"
                                   href="/book.do?type=detail&id=${i.id}">查看</a>
                                <a class="link-update btn btn-warning btn2"
                                   href="/book.do?type=update&id=${i.id}">修改</a>
                                <a class="link-del btn btn-danger btn2" onclick="detete_book(${i.id},this)">删除</a>
                                <a class="link-return btn btn-info btn2" onclick="return_book(${i.id},this)">归还</a>
                                <a class="link-borrow btn btn-primary btn2" onclick="borrow_book(${i.id},this)">借阅</a>
                            </td>
                        </tr>
                        </c:forEach>
        ...
        <!-- 页面底部 -->
        <footer>
            <p>版权所有 &copy; 2024 李钦淮 Library System</p>
        </footer>
</body>
</html>
...
<script>
    ...
    //TODO 借阅图书
    function borrow_book(id, obj) {
        layer.confirm('确认要借阅吗？', function (index) {
            $.get("/book.do", {id: id, type: 'borrow'}, function (data) {
                ...
                if (data.success) {
                    ...
                    layer.msg('借阅失败: ' + data.message, {icon: 2, time: 1000});
                }
            }).fail(function (xhr, status, error) {
                console.error("AJAX Error: " + status + error); // 添加错误处理
            });
        });
    }
    ...
    // 归还图书
    function return_book(id, obj) {
        layer.confirm('确认要归还吗？', function (index) {
            $.get("book.do", {id: id, type: 'return'}, function (data) {
                console.log(data); // 添加调试信息
                console.log(data.success); // 添加调试信息
                if (data.success) {
                    ...
                    layer.msg('归还失败: ' + data.message, {icon: 2, time: 2000});
                }
            });
        });
    }
